...
// 🔹 Holt Song-Daten und setzt Artist-Id korrekt
const loadSong = async () => {
  ...
  // WICHTIG: ArtistId korrekt setzen, da wir jetzt ein Artist-Objekt bekommen
  artistId.value = data.artist?.id ?? "";
};

// 🔹 Aktualisiert Song am Server
const updateSong = async () => {
  ...
  await axios.put(`http://localhost:8080/api/songs/${route.params.id}`, {
    ...
    artist: { id: artistId.value } // Backend erwartet komplettes Artist-Objekt
  });
